...
document.addEventListener('DOMContentLoaded', function () {
    $departmentTable = $("#tblTeamList").dataTable({
        "processing": true,
        "serverSide": true,
        "sorting": true,
        ...
        "columns": [
            ...
            },
            {
                ...
                className: 'text-center',
                "render": function (data, type, row) {
                    return '<a class="btn btn-sm btn-outline-secondary" href="/Admin/Team/ManageTeam?id=' + data.id + '"><i class="fa-solid fa-pen-to-square"></i></a> '
...
$(document).on('change', '.toggle-status', function () {
    var id = $(this).data('id');
    $.ajax({
        url: $('#tblTeamList').data('togglestatus') + "?id=" + id,
        method: 'GET',
...
$(document).on('click', '#btn-Delete', function (e) {
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                debugger;
                var url = $('#tblTeamList').data('delete') + "?id=" + id;;
                $.ajax({
                    url: url,
                    type: 'DELETE',
                    success: function (response) {
