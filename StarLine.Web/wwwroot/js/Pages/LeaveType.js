...
document.addEventListener('DOMContentLoaded', function () {
    $departmentTable = $("#tblLeaveType").dataTable({
        "processing": true,
        "serverSide": true,
        "sorting": true,
        ...
        "columns": [
            { "data": "typeName", "name": "typeName", "autoWidth": true, orderable: false, },
            { "data": "description", "name": "description", "autoWidth": true },
            { "data": "maxDaysPerMonth", "name": "maxDaysPerMonth", "autoWidth": true },
            { "data": "isCarryForward", "name": "isCarryForward", "autoWidth": true },
            {
                data: "genderContraint", // Important: set data to null for custom HTML
                orderable: false,
                searchable: false,
                className: 'text-center',
                "render": function (data, type, row) {
                    return row.genderContraint == 0 ? "No" : "yes";
                }
            },
            {
            ...
            },
            {
                ...
                className: 'text-center',
                "render": function (data, type, row) {
                    return '<a class="btn btn-sm btn-outline-secondary" href="/Admin/LeaveType/ManageLeaveType?id=' + data.id + '"><i class="fa-solid fa-pen-to-square"></i></a> '
...
$(document).on('change', '.toggle-status', function () {
    var id = $(this).data('id');
    $.ajax({
        url: $('#tblLeaveType').data('togglestatus') + "?id=" + id,
        method: 'GET',
...
$(document).on('click', '#btn-Delete', function (e) {
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                debugger;
                var url = $('#tblLeaveType').data('delete') + "?id=" + id;;
                $.ajax({
                    url: url,
                    type: 'DELETE',
                    success: function (response) {
