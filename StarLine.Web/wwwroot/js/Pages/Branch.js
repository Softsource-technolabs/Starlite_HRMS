...
document.addEventListener('DOMContentLoaded', function () {
    $departmentTable = $("#tblBranchList").dataTable({
        "processing": true,
        "serverSide": true,
        "sorting": true,
        ...
        "columns": [
            { "data": "branchCode", "name": "branchCode", "autoWidth": true, orderable: false, },
            { "data": "branchName", "name": "branchName", "autoWidth": true, orderable: false, },
            { "data": "city", "name": "city", "autoWidth": true, orderable: false, },
            { "data": "contactNumber", "name": "contactNumber", "autoWidth": true, orderable: false, },
            ...
            },
            {
                ...
                className: 'text-center',
                "render": function (data, type, row) {
                    return '<a class="btn btn-sm btn-outline-secondary" href="/Admin/Branch/ManageBranch?id=' + data.id + '"><i class="fa-solid fa-pen-to-square"></i></a> '
...
$(document).on('change', '.toggle-status', function () {
    var id = $(this).data('id');
    $.ajax({
        url: $('#tblBranchList').data('togglestatus') + "?id=" + id,
        method: 'GET',
...
$(document).on('click', '#btn-Delete', function (e) {
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                debugger;
                var url = $('#tblBranchList').data('delete') + "?id=" + id;;
                $.ajax({
                    url: url,
                    type: 'DELETE',
                    success: function (response) {
